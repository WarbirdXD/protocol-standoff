...
public class FirebaseSpawnAnalytics : MonoBehaviour
{
    ...
    public class SpawnDataPoint
    {
        ...
        public string quality; // Excellent, Good, Fair, Poor, Terrible
    ...
    {
        public string locationKey; // Grid cell identifier
    ...
    void Update()
    {
        if (!firebaseInitialized || !enableCloudSync) return;
    ...
    /// <summary>
    /// Record a spawn event (called by SpawnQualityAnalyzer)
    /// </summary>
    public void RecordSpawnEvent(Vector3 position, float timeToFirstDamage, float survivalTime, bool wasSpawnKill, string quality)
    {
        ...
        var dataPoint = new SpawnDataPoint(
            mapId,
            position,
            timeToFirstDamage,
            survivalTime,
        ...
        // If too many pending, upload immediately
    ...
    /// <summary>
    /// Upload pending spawn data to Firebase
    /// </summary>
    private async void UploadPendingData()
    {
        if (!firebaseInitialized || pendingUploads.Count == 0) return;
        
        Debug.Log($"📤 Uploading {pendingUploads.Count} spawn data points to Firebase...");
        
        try
        {
            // Group by location for efficient aggregation
            ...
            // Upload aggregated data for each location
    ...
    /// <summary>
    /// Upload data for a specific location
    /// </summary>
    private async Task UploadLocationData(string locationKey, List<SpawnDataPoint> dataPoints)
    {
        ...
        {
            // Create new data
            locationData = new CloudSpawnLocationData
            {
                locationKey = locationKey
            };
        }
        
        // Aggregate new data points
        foreach (var point in dataPoints)
        {
            // Update running averages
            float totalWeight = locationData.totalSpawns;
            
            locationData.avgTimeToFirstDamage = 
                (locationData.avgTimeToFirstDamage * totalWeight + point.timeToFirstDamage) / (totalWeight + 1);
            
            locationData.avgSurvivalTime = 
                (locationData.avgSurvivalTime * totalWeight + point.survivalTime) / (totalWeight + 1);
        ...
        float spawnKillRate = locationData.totalSpawns > 0 ? 
            (float)locationData.spawnKills / locationData.totalSpawns : 0f;
    ...
    /// <summary>
    /// Download cloud data from Firebase
    /// </summary>
    ...
    /// <summary>
    /// Get cloud quality score for a position
    /// </summary>
    ...
    /// <summary>
    /// Check if location has cloud data indicating spawn kill zone
    /// </summary>
    public bool IsCloudSpawnKillZone(Vector3 position)
    {
        ...
        if (cloudData.ContainsKey(locationKey))
        {
            ...
            if (data.totalSpawns >= 5)
            {
                ...
                return spawnKillRate > 0.3f; // > 30% spawn kill rate
            }
        }
    ...
    /// <summary>
    /// Get cloud statistics for a position
    /// </summary>
    ...
    /// <summary>
    /// Get total spawns tracked across all locations
    /// </summary>
    ...
    /// <summary>
    /// Get location key for a position (5m grid)
    /// </summary>
    ...
    /// <summary>
    /// Get analytics report including cloud data
    /// </summary>
    public string GetCloudAnalyticsReport()
    {
        ...
        var spawnKillZones = cloudData.Values.Where(d => 
            d.totalSpawns >= 5 && (float)d.spawnKills / d.totalSpawns > 0.3f
        ).ToList();
    ...
    {
        // Upload any remaining data before quitting
