...
public class SpawnQualityAnalyzer : MonoBehaviour
{
    ...
    /// <summary>
    /// Record for a single spawn event
    /// </summary>
    ...
    /// <summary>
    /// Statistics for a specific spawn location
    /// </summary>
    private class SpawnLocationStats
    {
        ...
        public void UpdateQuality()
        {
            ...
            float survivalScore = Mathf.Clamp01(averageSurvivalTime / 30f) * 50f; // 30s = perfect
            float engagementScore = Mathf.Clamp01(averageTimeToFirstDamage / 8f) * 50f; // 8s = perfect
    ...
    /// <summary>
    /// Active spawn instance being tracked
    /// </summary>
    ...
    {
        Excellent,  // > 8s to engagement, > 20s survival
        Good,       // 5-8s to engagement, > 10s survival
        Fair,       // 3-5s to engagement
        Poor,       // < 3s to engagement (spawn kill)
        Terrible    // Instant death
    }
    ...
    /// <summary>
    /// Register damage taken (for time-to-first-damage tracking)
    /// </summary>
    public void RegisterDamage(GameObject player)
    {
        if (!enableTracking || !activeSpawns.ContainsKey(player)) return;
    ...
    /// <summary>
    /// Register death (completes spawn tracking)
    /// </summary>
    ...
    /// <summary>
    /// Update statistics for a spawn location
    /// </summary>
    private void UpdateLocationStats(SpawnRecord record)
    {
        ...
        {
            locationStats[gridCell] = new SpawnLocationStats
            {
                gridCell = gridCell
            };
        }
    ...
    /// <summary>
    /// Get grid cell for a position (for heatmap)
    /// </summary>
    ...
    /// <summary>
    /// Get quality score for a position (used by spawn system)
    /// </summary>
    ...
    /// <summary>
    /// Check if location has history of spawn kills
    /// </summary>
    ...
    public string GetAnalyticsReport()
    {
        ...
        var qualityCounts = spawnHistory.GroupBy(s => s.quality)
    ...
    /// <summary>
    /// Visualize spawn quality heatmap
    /// </summary>
    private void OnDrawGizmos()
    {
        if (!showHeatmap || locationStats.Count == 0) return;
